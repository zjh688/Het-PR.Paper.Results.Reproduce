# Personalized Linear Regression using covariate distance matching
# Adapted by Caleb Ellington from Ben Lengerich's https://github.com/blengerich/Personalized_Regression
import numpy as np
...
#             print(pca.explained_variance_ratio_)
    ...
    def _maybe_log_status(self, iteration, losses):
        if self.log is not None and iteration % self.verbosity == 0:
    ...
    def fit(self, X, Y, U, dU, delta_U=None, neighborhoods=None,
        ...
        verbosity=100, hierarchical=False,
        kd_leafsize=None, calc_neighbors=True,
        record_distances=False, log_file=None):
        ...
        log = open(log_file, 'a') if log_file is not None else None
        if hierarchical:
...
class DistanceMatchingRegressor:
    """
    U (N x K): covariates
    X (N x P): features
    Y (N x 1): response
    
    Theta (N x P) := Q^T Z: parameter estimates for personalized linear models Y[i] = Theta[i] * X[i]
    Rho: penalty on personalized linear models (L1 or L2)
    
    delta_U (N x N): pairwise distance transform between U[i] and U[j], calculated as Phi * dU(U[i], U[j])
    dU (list of K functions, default absolute difference): pairwise distance metrics on each feature in U
    Phi (K x 1): learned linear coefficients for each function dU
    Upsilon (hyperparameter): weight of L1 or L2 regularization on Phi
    Psi: value of penalty on covariate transform weights, calculated as Upsilon * reg(Phi)
    Gamma (hyperparameter): weight of distance-matching penalty: sum(||Z[i] - Z[j]|| - delta_U[i,j])
    """
    def __init__(self, n_neighbors=5, gamma=1e6, upsilon=1e-3, latent_dim=np.inf):
        self.n_neighbors = n_neighbors
        self.gamma = gamma
        self.upsilon = upsilon
        self.latent_dim = latent_dim
        # Linear loss
        self.f = lambda x, y, w: .5 * (y - x.dot(w)) ** 2
        self.f_prime = lambda x, y, w: (-x) * (y - x.dot(w))
        # Lasso penalty on regression weights
        self.rho_w = lambda w, i: np.linalg.norm(w, ord=1)
        self.rho_w_prime = lambda w, i: np.sign(w)
        # Lasso penalty on covariate transform
        self.psi_u = lambda phi_u: upsilon * np.linalg.norm(phi_u, ord=1)
        self.psi_u_prime = lambda phi_u: upsilon * np.sign(phi_u)
        # Pairwise covariate distance metric
        self.dU_metric = lambda x,y: np.abs(x-y)


    def fit(self, x, y, u, lr=1e-3, epochs=20000, silent=True):
        self.u_dim = u.shape[1]
        self.x_dim = x.shape[1]
        self.init_phi_u = np.ones(self.u_dim) / self.u_dim
        self.dU = [self.dU_metric] * self.u_dim
        verbosity = np.inf if silent else 100
        self.u_train, self.x_train, self.y_train = u, x, y
        self.latent_dim = min(self.latent_dim, x.shape[1])
        # Build the distance matching model
        self.pop_w = (np.linalg.inv(x.T @ x) @ x.T @ y).T
        self.init_w = np.tile(self.pop_w, (len(x), 1))
        self.model = DistanceMatchingModel(self.init_w, self.f, self.f_prime, 
                                     self.gamma, self.latent_dim,
                                     n_neighbors=self.n_neighbors,
                                     update_ztree_every=10,
                                     calc_dist_errors_every=1,
                                     calc_closest_every=10,
                                     rho_beta=self.rho_w,
                                     rho_beta_prime=self.rho_w_prime,
                                     init_phi_u=self.init_phi_u,
                                     psi_u=self.psi_u,
                                     psi_u_prime=self.psi_u_prime,
                                     intercept=False,
                                     n_threads=1)
        z_dmr, q_dmr = self.model.fit(x, y, u, self.dU, 
                                      init_lr=lr, max_iters=epochs,
                                      verbosity=verbosity,
                                     )
        self.w_hat = z_dmr @ q_dmr

        return self.w_hat, np.ones_like(self.w_hat)
    
    def predict_w(self, u):
        pred_ws = np.zeros((len(u), self.x_dim))
        distmat_u = np.zeros((len(u), len(self.u_train), self.u_dim))
        for i in range(len(u)):
            for j in range(len(self.u_train)):
                for k in range(self.u_dim):
                    distmat_u[i,j,k] = self.dU[k](self.u_train[j,k], u[i,k])
        for i in range(len(u)):
            closest_z = np.argsort(distmat_u[i] @ self.model.best_phi_u)
            pred_ws[i] = np.mean(self.w_hat[closest_z[:self.n_neighbors]], axis=0)
        return pred_ws

    def predict_y(self, x, u):
        w_hat = self.predict_w(u)
        y_hat = np.array([x[i].dot(w_hat[i]) for i in range(len(u))])
        return y_hat
    
    def mse(self, x, y, u):
        y_hat = self.predict_y(x, u)
...
if __name__ == '__main__':
    ...
    u = np.linspace(-1, 1, n)
    w_1 = u.copy()
    w_2 = np.concatenate((w_1[:len(w_1)//2]**2 - 1, 1 - w_1[len(w_1)//2:]**2), axis=0)
    ...
    y = np.sum(w * x, axis=1)[:, np.newaxis] + epsilon
    u_train, x_train, y_train = u[:-10], x[:-10], y[:-10]
    u_test, x_test, y_test = u[-10:], x[-10:], y[-10:]
    dmr = DistanceMatchingRegressor()
    dmr.fit(x_train, y_train, u_train, epochs=1)
    print(f"Starting MSE {dmr.mse(x_test, y_test, u_test)}")
    dmr.fit(x_train, y_train, u_train, epochs=20000)
    print(f"Final MSE {dmr.mse(x_test, y_test, u_test)}")
